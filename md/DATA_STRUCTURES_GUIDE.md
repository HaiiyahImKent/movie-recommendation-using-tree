...
**CinePath** is a sophisticated movie recommendation system that combines three fundamental data structures:

-   **Binary Decision Tree** (409 nodes, 12 levels) - Core recommendation algorithm
...
All computation happens **client-side** for instant performance (<1ms for recommendations).
...
1. [Decision Tree - Deep Dive](#decision-tree---deep-dive)
2. [Queue Implementation](#queue-implementation)
3. [Stack for Undo](#stack-for-undo)
4. [Performance Timing](#performance-timing-with-performancenow)
...
| Space Complexity | O(h) = O(12) | Recursion call stack depth at max              |
...
localStorage persists: [Session11, Session10, ..., Session2]
Badge shows: "10"
```
...
```typescript
// Layer 1: Check localStorage for recent duplicate
...
// Layer 2: Set ref flag to prevent same-session save
...
// Layer 3: On replay, set flag to skip save
if (isReplayingRef.current) {
	// Skip the save effect entirely
	return;
}

// Layer 4: New navigation resets all flags
useEffect(() => {
...
1. V8/SpiderMonkey JIT Compilation
   - Code compiled to machine code
   - No interpretation overhead

2. Object Lookup Optimization
   - 409 nodes all in memory (CPU cache friendly)
   - Binary tree navigation = pointer dereference (~1-2 ns per node)

3. No I/O Operations
   - Pure computation (no disk, no network)
   - No synchronous blocking

4. CPU Speed
   - Modern CPUs: ~3 GHz = 3 billion ops/sec
   - 7 nodes × 2 operations = 14 ops = ~5 nanoseconds
   - + overhead = ~0.00023 milliseconds
```
...
-   **Props:**
    -   `visitedNodes`, `traversalTimeMs`, `depth`
    -   `totalTreeNodes`, `treeHeight`
    -   `selectedGenreNames`, `movieCount`
-   **Displays:**
...
│ ┌───────────────────────────────────────────────────────┐   │
│ │ DecisionTree initialized                              │   │
│ │ - 409 nodes loaded in memory                          │   │
│ │ - Root node ready                                     │   │
...
│ └─ Display Q2                                               │
│                                                             │
│ User answers Q2-Q7 (6 more times)                           │
│ └─ Reach leaf node with recommendedGenres = [14, 18, 12]   │
│                                                             │
│ User clicks "Finish"                                        │
│ ├─ Measure time: performance.now() - startTime             │
│ ├─ Calculate stats:                                         │
│ │  ├─ visitedNodes: 7                                       │
│ │  ├─ traversalTimeMs: 0.000234                             │
│ │  ├─ depth: 7                                              │
...
│ └─ Navigate to /results with state containing genres & stats│
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
...
│ │ 2. Query TMDB with genres = [14, 18, 12]              │   │
│ │    → Returns ~50 movies                               │   │
│ │ 3. Rank movies by genre closeness                     │   │
│ │ 4. Format movie data (title, image, etc.)             │   │
│ │ 5. setMovies(formattedMovies)                          │   │
│ └───────────────────────────────────────────────────────┘   │
│                                                             │
│ Display:                                                    │
│ ├─ RecommendationStats (original stats from tree)          │
│ ├─ 50 movies in grid                                       │
│ └─ "← New Recommendation" button                            │
│                                                             │
│ On movies change:                                           │
│ ├─ Check if session already saved (localStorage)            │
│ ├─ Check if currently replaying (isReplayingRef)            │
│ ├─ If both false: saveSessionToHistory()                    │
│ │  ├─ Create SessionItem with:                              │
│ │  │  ├─ id, timestamp, genres, movieCount                  │
│ │  │  ├─ genreNames (mapped from genre IDs)                 │
│ │  │  ├─ movies (full array for replay)                     │
│ │  │  └─ stats (original DFS stats)                         │
│ │  ├─ Get saved sessions from localStorage                  │
│ │  ├─ Prepend new session: [newSession, ...saved]           │
│ │  ├─ Keep only 10: .slice(0, 10)                           │
│ │  ├─ Save to localStorage                                  │
│ │  ├─ Dispatch "cinepath-session-saved" event               │
│ │  └─ Mark hasSessionSavedRef = true                        │
│ └─ Update session count badge in SessionHistory             │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│ SESSION HISTORY PANEL (Floating Button)                     │
│ ┌───────────────────────────────────────────────────────┐   │
│ │ Badge shows: "1" (one session saved)                  │   │
│ │ User clicks history button → Panel opens               │   │
│ │                                                        │   │
│ │ Recent Sessions:                                       │   │
│ │ ┌──────────────────────────────────────────────┐     │   │
│ │ │ #1 | Dec 5, 3:45 PM                         │     │   │
│ │ │     [Fantasy] [Drama] [Adventure]            │     │   │
│ │ │     50 movies found                 [Delete] │     │   │
│ │ └──────────────────────────────────────────────┘     │   │
│ │                                                        │   │
│ │ User clicks session #1                                │   │
│ │ ├─ Dispatch "cinepath-replay-session" event           │   │
│ │ ├─ Close history panel                                │   │
│ │ └─ Results.tsx replay handler triggers                │   │
│ └───────────────────────────────────────────────────────┘   │
│                                                             │
│ Results Page Updates:                                       │
│ ├─ setMovies(session.movies)  → Show saved 50 movies       │
│ ├─ setStats(session.stats)    → Show original stats        │
│ ├─ setCurrentSessionId(id)    → Mark as replaying          │
│ ├─ setCurrentSessionGenres(genres) → Update preference     │
│ ├─ isReplayingRef = true      → Prevent duplicate save     │
│ └─ Display original Algorithm Performance stats            │
└─────────────────────────────────────────────────────────────┘
                            ↓
│ User sees:                                                  │
│ ├─ Original stats (Time: 0.000234ms, Nodes: 7, Depth: 7)  │
│ ├─ Same 50 movies from that session                        │
│ ├─ Genres showing original selection                       │
│ └─ "Viewing Session #1" indicator (if shown)               │
                            ↓
│ User clicks "← New Recommendation"                          │
│ ├─ Navigate back to /recommend (questionnaire)             │
│ ├─ Reset all session tracking                              │
│ └─ Ready for next recommendation                           │
│                                                             │
│ OR User makes another recommendation                        │
│ ├─ New DFS traversal (7 nodes, different path)              │
│ ├─ Navigate to /results                                     │
│ ├─ Show new 50 movies                                       │
│ ├─ Save as Session #2                                       │
│ └─ History badge updates: "2"                               │
└─────────────────────────────────────────────────────────────┘
```
...
## For Your Saturday Presentation
...
This is production-quality code suitable for real-world deployment, with robust duplicate prevention, session persistence, and a seamless user experience.
